...
for log_path in log_files:
    ...
    with open(log_path) as f:
        for line in f:
            if "ops/s" in line:
                ...
                parts = line.strip().split()
                try:
                    # Commit throughput value is the 2nd column
    ...
    if commit_throughputs:
        median_commit_val = statistics.median(sorted(commit_throughputs))
        print(f"{node} median {median_commit_val:.0f} commit/s")
        total_commit_throughput += median_commit_val
    else:
